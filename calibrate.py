...
def main(image_files,
    ...
    fisheye:
        set to True to use fisheye camera model
    pattern_size:
    ...
    json_file:
        JSON file to write calibration data to
    debug_dir:
        if set,
        the path to which debug images with the detected chessboards are written
    """
    ...
    def process_image(fname):
        ...
        if w != img.shape[1] or h != img.shape[0]:
            return (fname,
                    "Size %dx%d doesn't match" % (img.shape[1], img.shape[0]))
    ...
    if threads <= 1:
        ...
        sys.stdout.write('Processing %d images using %d threads ' %
                         (len(image_files), threads))
    ...
    print('Found chessboards in %d out of %d images' %
          (num_chessboards, len(image_files)))
    print()

    if num_chessboards == 0:
        print(
            'No chessboards to use! Was the correct chessboard size set using the -c option?'
        )
        sys.exit(-1)
    ...
    for cb_index in range(num_chessboards):
        img_points2, _ = project_func(obj_points[cb_index], rvecs[cb_index],
                                      tvecs[cb_index], camera_matrix,
                                      dist_coefs)
        error = cv2.norm(img_points[cb_index], img_points2,
    ...
    print()
    print("Average reprojection error: %.6f +/- %.6f" %
          (reprojection_error_avg, reprojection_error_stddev))
    ...
    if sensor_size is not None:
        ...
        (fovx, fovy, focal_length, principal_point,
         aspect_ratio) = cv2.calibrationMatrixValues(camera_matrix, (w, h),
                                                     sensor_size[0],
                                                     sensor_size[1])
    ...
    if debug_dir is not None:
        ...
        for fname in image_files:
            ...
            outfile2 = os.path.join(debug_dir,
                                    name + '_undistorted_cropped.png')
...
''')
    ...
    main(image_files, fisheye, corners, square_size, threads, json_file,
         debug_dir)
